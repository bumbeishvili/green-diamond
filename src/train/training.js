...
// it); leave it and it goes. Puzzle crates round the complex hold one too. Fourteen kinds, dealt in a
// shuffled round so they all come up before any comes again. Each has a level of its own, 1 to 10: a
// good round moves it up, a poor one down. A round pays by its score, up to 300 lari (600 later in
// the match); three good rounds running, a gun you haven't got (or a power-up); every ten good rounds,
// the gun in your hands a level up.
...
export const MODES = { off: 0, light: 3, normal: 1, intense: 1 };   // an offer every n waves (0: never)
...
export class Training {
  ...
  offer() {
    ...
    this.offerN = this.mode === 'intense' ? 2 : 1;
    el.querySelector('span').innerHTML = this.offerN === 2 ? `two puzzles, up to <em>+${this.worth()}</em> lari each` : `a puzzle for up to <em>+${this.worth()}</em> lari`;
  ...
  take() { this.hideOffer(); this.play('Brain training', this.offerN || 1); }
  ...
  crate() {
    ...
    this.play('Puzzle crate', 1);
  }

  // n puzzles (or one of your choosing, for practice: nothing paid, nothing in the game changes)
  async play(title, n = 1, only = null) {
    if (this.open) return;
    const practice = !!only;
    this.busy = true;
    ...
    try {
      for (let k = 0; k < n; k++) {
        ...
        const res = await this.card.play(meta, level, { index: k, count: n });
        if (!res) break;
        const next = this.record(id, res.score);
        const gain = practice ? 0 : this.pay(res.score);
        gained += gain;
    ...
    // every ten good rounds: the gun in your hands, a level up
    const m = Math.floor(this.s.good / 10);
    if (!practice && m > (this.s.milestone || 0)) { this.s.milestone = m; this.save(); this.upgrade(`${m * 10} good rounds`); }
  }
  ...
  test(id, level = null) { if (level) this.s.levels[id] = Math.max(1, Math.min(10, level)); return this.play('Test', 1, id); }
  ...
  // ---- what it's worth ----
  pay(score) {
    const pts = Math.round((this.worth() * score) / 10) * 10;
    if (pts > 0) this.cash(pts);
    if (score >= 0.8 && this.s.streak > 0 && this.s.streak % 3 === 0) this.bonus(`${this.s.streak} good rounds running`);
    return pts;
  }
  ...
  panel() {
    ...
    if (!el) {
      ...
      el.addEventListener('click', async (e) => {
        ...
        if (b.dataset.play) { el.classList.add('hidden'); await this.play('Practice', 1, b.dataset.play); this.panel(); }
      });
    }
    const s = this.s;
